 ...
 *
 */
public class KlientFrame extends TableFrame{
	
	private static final long serialVersionUID = 1L;
	List<Rabat> rabatLista = new ArrayList<Rabat>();
	
	@Override
	public void addEditors() {
		
		GenericDAO dao = new GenericDAO();
		List<Object> rabatLista = dao.getAll("Rabat");
		TableColumn col = tbl.getColumnModel().getColumn(6);
		col.setCellEditor(new DefaultCellEditor(new JComboBox(rabatLista.toArray())));
		
		List<Object> ulica = dao.getAll("Ulica");
		col = tbl.getColumnModel().getColumn(7);
